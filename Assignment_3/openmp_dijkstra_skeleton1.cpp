...
void dijkstra(int N, int p, int *mat, int *all_dist, int *all_pred) {
    ...
    loc_min = (int *) malloc(p * sizeof(int));
    loc_u = (int *) malloc(p * sizeof(int));
    ...
    #pragma omp parallel num_threads(p)
    {
        for (int i = 1; i < utils::N; i++) {
            ...
            loc_min[threadnum] = my_min;
            loc_u[threadnum] = my_u;
            ...
            if (threadnum == 0){
                ...
                for (int x = 0; x < p; x++){
                    if(loc_min[x] < global_min){
                        global_min = loc_min[x];
                        u = loc_u[x];
                    }
                }
            }
